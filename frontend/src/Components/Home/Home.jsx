...
import { LeftOutlined, RightOutlined, StarFilled, MessageOutlined, BookOutlined, ShareAltOutlined } from '@ant-design/icons';
...
const HeroCarousel = () => {
  ...
  const herocarouselItems = [
    ...
    },
    {
      bgImage: bg3,
      title: "Community",
      description: "Browse Pre-built PCs by passionate builder like you over our community, engage with people's builds and share reviews!",
    ...
    },
    {
      ...
      description: "Search for your desired component with filtered search.\n\nMeet your personal AI Assistant, get expert real-time recommendations for building the perfect PC, tailored to Your needs and budget!",
  ...
  const handleDragEnd = () => {
    ...
    if (Math.abs(currentX) > 50) {
      if (currentX > 0) {
        setActiveIndex(prev => (prev === 0 ? herocarouselItems.length - 1 : prev - 1));
      } else {
        setActiveIndex(prev => (prev === herocarouselItems.length - 1 ? 0 : prev + 1));
      }
    }
    setCurrentX(0);
  };
  ...
  const getItemStyle = (index) => {
    ...
    else if (index === activeIndex + 1 || (activeIndex === herocarouselItems.length - 1 && index === 0)) {
      return {
  ...
  return (
    ...
    >
      ...
      <div className="herobackground-container">
        <div 
          className="herobackground-image"
          style={{ 
            backgroundImage: `url(${herocarouselItems[activeIndex].bgImage})`,
      ...
      <div className="herocontent-panel">
        <h1 className="herocarousel-title">{herocarouselItems[activeIndex].title}</h1>
        <p className="herocarousel-description">{herocarouselItems[activeIndex].description}</p>
        <Button 
          className="herocarousel-button"
          style={{ backgroundColor: herocarouselItems[activeIndex].buttonColor }}
      ...
      <div className="herocarousel-items-container">
        {herocarouselItems.map((item, index) => (
          <div
            key={index}
            className={`herocarousel-item ${index === activeIndex ? 'active' : ''}`}
...
const VerticalCardCarousel = () => {
  ...
  const prevSlide = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 + prebuiltcardItems.length) % prebuiltcardItems.length);
  };

  const getCardStyle = (index) => {
    const position = (index - currentIndex + prebuiltcardItems.length) % prebuiltcardItems.length;
  ...
  return (
    <div className="vertical-carousel-container">
      <div className="prebuiltsection-header">
        ...
        <p className="prebuiltsection-subtitle">Personal Computers built to suit your needs</p>
      </div>
...
const FeaturedBuildsCarousel = () => {
  ...
  const prevSlide = () => {
    setCurrentIndex(prev => (prev - 1 + featuredBuilds.length) % featuredBuilds.length);
  };
  ...
  return (
    <div className="featured-builds-container">
      <div className="featuredBuildssection-header">
        ...
        <p className="featuredBuildssection-subtitle">Explore User builds with the highest rating</p>
      </div>
            ...
            {getVisibleCards().map((guidebuild, position) => (
              ...
              >
                ...
                <div className="featuredBuildsguidebuild-content">
                  <h3 className="featuredBuildsguidebuild-title">{guidebuild.title}</h3>
                  <p className="featuredBuildsguidebuild-specs">{guidebuild.specs}</p>
  ...
  return (
    <div className="community-section">
      <div className="commsection-header">
        ...
        <p className="commsection-subtitle">Explore other users' builds and share your own for mutual exchange of knowledge</p>
      </div>
      ...
      <div className="community-post-container">
        ...
        >
          <div className="commpost-image-container">
            <img src={postImage} alt="Gaming PC Build" className="commpost-image" />
          </div>
          <div className="commpost-details">
            
            <h3 className="commbuild-title">Build Name</h3>
            
            <p className="commbuild-content">
              "Description"
            </p>
