      ...
      vectorStore = await Chroma.fromExistingCollection(embeddings, {
        ...
        embeddingFunction: {
          generate: async (texts) => {
            const embs = await Promise.all(texts.map(text => embeddings.embedQuery(text)));
            return embs;
          }
        }
      });
 ...
 */
const getChatHistory = async (sessionId, req) => {
  try {
 ...
 */
const updateChatHistory = async (sessionId, message) => {
  try {
 ...
 */
const associateSessionWithUser = async (sessionId, userId) => {
  try {
 ...
 */
const getUserContext = async (req, sessionId) => {
  try {
    const chatHistory = await getChatHistory(sessionId, req);
    let userInfo = null;
 ...
 */
const getComponents = async (componentType, filters = {}) => {
  try {
 ...
 */
const getRelevantKnowledge = async (query) => {
  try {
    if (!vectorStore) {
 ...
 */
const checkComponentCompatibility = async (components) => {
  ...
  try {
    ...
    for (const componentType of componentTypes) {
      try {
        ...
        const components = await getComponents(componentType, filters);
    ...
    const compatibilityResults = await checkComponentCompatibility(componentData);
